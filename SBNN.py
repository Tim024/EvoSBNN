...
class SparseBinaryLayer:
    def __init__(self, input_size, output_size, density=0.1, op=np.bitwise_and):
        ...
        self.weights = np.array(np.zeros((output_size, input_size)),
                                dtype=bool)  # Row = output neuron, Column = input neuron, weight(out,in) = True if link
        ...
        for input in input_batch:
            ...
            for neuron in self.weights:
                output.append(True if np.sum(
                    self.operation(input, neuron)) > 2 else False)  # Activate only if at least 2 inputs are true
